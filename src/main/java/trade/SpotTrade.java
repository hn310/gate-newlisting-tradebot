    ...
    public void createSellOrder(String currencyPair, String sellAmount, String sellPrice) throws ApiException {
        do {
            try {
                ...
                if (gae.getErrorLabel().equals("INVALID_CURRENCY") || gae.getErrorLabel().equals("SERVER_ERROR")
                        || gae.getErrorLabel().equals("TOO_MANY_REQUESTS")) {
                    continue;
                }
            } catch (Exception e) {
        ...
        do {
            try {
                ...
                for (BatchOrder bo : batchOrders) {
                    logger.info("side: " + bo.getSide() + ", text: " + bo.getText() + ", succeeded: "
                            + bo.getSucceeded() + ", label: " + bo.getLabel() + ", message: " + bo.getMessage()
                            + ", create_time_ms: " + bo.getCreateTimeMs() + ", amount: " + bo.getAmount() + ", price: "
                            + bo.getPrice() + ", filled price: " + bo.getFillPrice() + ", filled total: "
                            + bo.getFilledTotal());
                }
            } catch (GateApiException gae) {
                logger.error(gae.getMessage());
